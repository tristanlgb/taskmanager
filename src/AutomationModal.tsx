...
export function AutomationModal({settings,message,onSave,onClose}:{settings:AutomationSettings;message:string;onSave:(settings:AutomationSettings)=>void;onClose:()=>void}) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal small" onMouseDown={event=>event.stopPropagation()}>
    <div className="modal-head"><div><h2>Automatización con n8n</h2><p>Priorización externa activa en tu n8n Cloud.</p></div><button type="button" aria-label="Cerrar" onClick={onClose}><X/></button></div>
    ...
    <div className="modal-footer"><button type="button" className="secondary" onClick={onClose}>Cerrar</button><button type="button" className="primary" onClick={()=>onSave(draft)}><Send/>Guardar conexión</button></div>
  </div></div>;
}
